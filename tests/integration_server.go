...
func (mockBackendBuilder) New(cfg *Config) http.Server {
	...
	mux.HandleFunc("/param_forwarding/", checks.XForwardedFor(http.HandlerFunc(payloads.EchoEndpoint)))
	...
	mux.HandleFunc("/delayed/", checks.XForwardedFor(behaviors.Delayed(cfg.getDelay(), http.HandlerFunc(payloads.EchoEndpoint))))
